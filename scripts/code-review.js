...
class CodeReviewAgent {
  constructor() {
    this.errors = [];
    this.warnings = [];
    // Skip this file itself: its detection patterns (e.g. the debugger check)
    // match their own source and produce false-positive blockers
    this.stagedFiles = getStagedFiles(['.ts', '.js']).filter(
      (file) => path.basename(file) !== 'code-review.js'
    );
