...
**RESOLVED (partially) — checked SRC-474's raw text directly** (`trm-vault/topics/charlie/willow-run/sources/raw/SRC-474.json`). SRC-474 is Chris's own working research notes (dated 2026-05-01, self-labeled "Status: Working Research Notes"), not an independent publication. Its own "Research Note" under Finding 1 reads verbatim: *"Sorensen describes the yacht sketch episode in detail in his 1956 memoir, My Forty Years with Ford. Cross-reference with Benson Ford Research Center archives for any surviving original sketches or correspondence from early 1941."*

**This is not a second, independent source — it traces to the same memoir V-5.3 is already sourced to.** The conflict is not "memoir vs. new source," it's "two different paraphrases of the memoir disagree on where the sketch happened" (hotel room in Draft v1.3's existing framing vs. yacht in this working-notes summary). Reading order changes accordingly:

- Most likely explanation: SRC-474 (a working-notes compilation, not a close read) paraphrased loosely or conflated two separate memoir details — Sorensen's known yacht ownership/Caribbean life (Section 7 material) and the San Diego hotel-room sketch scene are both real memoir content; a working summary could easily cross the wires.
- Alternative: the memoir's actual text places the sketch on the yacht, and whichever earlier source V-5.3's hotel-room framing derives from (not fully cited in the current draft — needs to be pinned down) is the one that's off.
- Either way: **neither reading clears BLOCKED.** This is still single-source (the memoir), now demonstrably unstable in secondary retelling, which if anything raises rather than lowers the bar for going to print with either version.

**Action required, narrowed:** pull the memoir's own text directly (not a paraphrase) for this passage — page reference not yet in hand from either this packet or V-5.3's existing sourcing notes — and confirm which location it actually states. This is now a single, well-defined lookup rather than an open-ended corroboration search. Until then, keep BOTH the hotel-room framing (5.3 prose) and the yacht claim (this packet) explicitly memoir-attributed and unconfirmed; do not let either harden into settled fact on the strength of a working-notes paraphrase.

Also relevant: **FCT-041** (SRC-473, confidence 0.95, promoted) confirms the San Diego/Consolidated trip itself ("In late 1940 or early January 1941, Charles Sorensen and Edsel Ford traveled to San Diego to tour Consolidated Aircraft's B-24 production facility") and **FCT-042** (SRC-473, promoted) corroborates the craft-friction detail already in Draft v1.3 beat 5.3/5.2 ("assembly conducted outdoors under the California sun on structural steel fixtures distorted by heat, making it impossible to produce two planes alike"). These two are from a different working-notes doc (SRC-473) than the yacht claim (SRC-474) and support the *existing* hotel-room framing's setup (the San Diego trip happened, the craft-culture collision was real) without resolving where the sketch itself was actually drawn.
